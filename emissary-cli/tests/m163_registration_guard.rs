//! Post-M152 corrective registration guard.
//!
//! This is intentionally separate from the long historical M062 helper chain.
//! It binds the current registration to the exact M165 zero-production
//! qualification budget and prevents completed M164 work from being mistaken
//! for active production authority.
...
fn m163_budget_stays_inside_i2pcontrol_policy_root() {
    ...
    let paths = manifest["current_registration"]["production_paths"]
        .as_array()
        .expect("M165 production paths");
    assert!(paths.is_empty(), "M165 must remain zero-production");
