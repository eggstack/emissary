...
- M137/M134 historical planning used “M138” only as the name of a possible NewDest corrective and explicitly recorded that it was not required. No M138 implementation plan was registered. This unrelated requalification milestone therefore uses M139 to preserve unambiguous historical references.
...
M139 is successful only if the repository can truthfully say:

1. the current `325/47/468` matrix is mechanically consistent with production behavior and active documentation;
2. M127–M130 shared-control-plane security guarantees still hold after lifecycle work;
3. M135 live quantity/LeaseSet control, M136 idle reduction, M137 idle close/reasoned teardown, and M134 proven-resume NewDest compose without violating earlier containment/security/runtime invariants;
...
If these conditions hold, M139 closure becomes the new current implemented-subset runtime/security qualification authority and supersedes M130 **only for current-head qualification**. Historical M130 closure remains immutable evidence for its original baseline.

M139 is not a Proposal capability milestone. It must not increase support counts merely to improve completeness.
...
Current evidence making M139 dependency-ready:

- `plans/registry.md` and M095 agree on `325/47/468`;
- M134 closure records the lifecycle roadmap complete;
- M134 closure reports core, I2PControl library, containment/matrix and live-runtime evidence green on the lifecycle implementation line;
...
- `emissary-cli/tests/m130_post_corrective_requalification.rs` still hard-codes `284/96/460` and contains a baseline-specific assertion that M130 changed no core/composition paths, historically valid for M130 but invalid as a current-head assertion after M135–M137;
- the full-support roadmap header is current while stale body text still describes `284/88/468` and 88 blocked residuals.

This is qualification/authority drift with a bounded correction surface, not an unresolved architecture dependency.
...
- `not_applicable` cells remain evidence-backed;
- full Proposal 170 support must not be claimed while applicable blockers remain.

M139 has a **zero-promotion budget**. A matrix count change is not an implementation objective.

If requalification proves an existing `apply` claim wrong, do not paper over it. Record the discrepancy, demote only if evidence requires it, and close M139 as corrective-required/blocked with a follow-up plan. A truthfulness demotion discovered by requalification is allowed; a convenience count change is not.
...
- M139 authorizes no new production path outside those accepted seams;
...
Requalify rather than merely inherit:
...
- add a tunnel type, action, RPC method, status, alias, or unrelated base-I2PControl method;
...
If a production defect is found that requires runtime code changes, M139 stops and records a corrective plan rather than expanding this qualification milestone into an implementation refactor.

## 5. Current drift inventory to reconcile

The implementation agent must first mechanically inventory every current-head failure before editing tests.
...
2. `emissary-cli/tests/m130_post_corrective_requalification.rs`
   ...
   - contains an M130-baseline containment check whose historical assertion is valid for the M130 diff but must not be applied as a current-head prohibition against accepted M135–M137 core/composition seams.
...
4. `emissary-cli/tests/m061_containment.rs`
   - remains current containment authority and must be checked against `061-containment-boundary.toml`; any mismatch is a real governance problem, not automatically test drift.
5. `emissary-cli/tests/m062_dependency_containment.rs`
   - contains accumulated milestone path exceptions and stale planning-status prose; reconcile without weakening dependency isolation.
6. M127/M128/M129 tests
   - retain behavior/security regressions; inspect only for baseline/path assertions that accidentally treat M130 as immutable current head.
...
M139 is planning/test/documentation work. **No production Rust source change is pre-authorized.**
...
- a new `emissary-cli/tests/m139_post_lifecycle_requalification.rs` if a dedicated durable current-head composition guard is the cleanest separation from historical M130 assertions.

Expected machine/planning/document paths:

- `plans/implementation/i2pcontrol-proposal-170/061-containment-boundary.toml` only if exact accepted-path evidence needs truthful reconciliation;
- `plans/implementation/i2pcontrol-proposal-170/062-dependency-containment.toml` only for planning/test bookkeeping or exact dependency/path evidence;
- `plans/implementation/i2pcontrol-proposal-170/095-full-support-matrix.toml` only if recomputation finds an actual authority inconsistency; expected count is unchanged;
- `plans/implementation/i2pcontrol-proposal-170/105-residual-option-audit.toml` only if active residual accounting rather than historical evidence requires reconciliation;
- `plans/implementation/i2pcontrol-proposal-170/110-completion-ledger.toml` for a post-M139 qualification marker if the ledger convention requires one;
- `plans/registry.md`;
...
- `plans/closure/i2pcontrol-proposal-170/139-closure.md` at closure.

Forbidden production changes for M139:
...
A required change to a forbidden production path is a stop condition and requires a separately registered corrective implementation plan.
...
1. record exact HEAD and worktree cleanliness;
...
5. enumerate every active document claiming a current matrix or current runtime/security authority;
6. run known historical/current-head guard suites individually and classify every failure as:
...
Refactor/rebase stale tests while preserving milestone evidence.
...
- assertions about **current** matrix/support/docs must read current M095 authority or assert `325/47/468` under M139;
- current containment is governed by M061/M062 current manifests and accepted later seams, not M130's old `no core changes since 9948...` rule;
- do not turn exact-path containment into prefix/glob approval;
- do not delete a failing guard without replacing its still-applicable invariant.

Add M139-specific durable current-head guards where cleaner than overloading historical files.

### WP3 — Integrated lifecycle composition requalification

Re-run and cross-check the full lifecycle chain as one composition:
...
At least one M139 integration test must exercise the conceptual sequence idle reduction → idle close → authoritative `IdlePolicy` removal → successful resume/NewDest decision using deterministic/fake runtime seams, not wall-clock sleeps.
...
Re-run M127–M130 invariants on current head:

- token lifetime and expiry outcome;
- bounded token/throttle state;
- bounded batch cardinality and sequential dispatch;
- per-element authentication and no intra-batch credential sharing;
...
Do not accept “unchanged since M130” as sufficient evidence; current-head tests must pass.
...
For every non-I2PControl production path changed from upstream:
...
- no path may be allowed solely to make M139 green;
- production source remains free of Proposal/admin vocabulary outside the policy root;
- Yosemite exact optional alias and `subtle` ownership remain isolated under M062;
- feature-disabled builds do not activate I2PControl-only dependencies/runtime behavior.

If M061/M062 reveal an unplanned production path, stop and classify it before manifest expansion.
...
- registry names M139 as current runtime/security qualification authority;
- full-support roadmap states `325/47/468` everywhere it describes current state and describes 47, not 88, residual blockers;
- post-M114 roadmap is clearly historical for M127–M130 and says M130 was superseded for current-head qualification by M139, without rewriting M130 history;
- implementation README names M139 current authority and M130 historical qualification lineage;
- active user/developer docs retain **partial** Proposal support wording;
- no active document says M130 is current-head authority after successful M139 closure;
...
Mechanically enumerate the remaining 47 blocked cells and ensure active roadmap residual prose matches M095.

Expected grouping at start:
...
M095 wins if exact row accounting differs; explain any discrepancy. No residual is registered for implementation by M139.
...
M139 adds no lifecycle machinery, but must re-prove:
...
- control cells/trackers are bounded with deterministic overload/eviction behavior;
...
Any failure is a production defect, not documentation drift.

## 9. Compatibility and migration

Expected compatibility effect: none.

M139 must not alter public JSON-RPC schema, Proposal names/types/actions, SAM/I2CP wire behavior, durable tunnel/control-state schema, secret-store format, Yosemite dependency revision, router configuration, or external operational behavior.

Test/document changes may rebase current authority from M130 to M139. Historical closure and implementation references remain valid. No migration is authorized or expected.

## 10. Focused regression requirements

At minimum retain/add deterministic evidence for:

1. M095 declared counts equal mechanical counts and equal `325/47/468`;
2. exactly 840 TunnelManager cells remain represented;
3. lifecycle promotions are exactly M136 21 + M137 14 + M134 6 over the M131 post-refreeze baseline, with no promotion attributed to M139;
4. active docs state current `325/47/468` or derive it from machine authority;
5. active docs retain partial-support wording;
6. M130 historical lineage remains referenced but not called current-head authority after M139 closure;
...
24. failed/cancelled resume preserves committed identity and retryability;
...
cargo test -p emissary-cli --no-default-features --features i2pcontrol \
  ...
  --test m139_post_lifecycle_requalification \
  --no-fail-fast
...
If no dedicated M139 test file is needed after refactoring, omit only that target and explain where the durable current-head guard lives.

Known stable/nightly rustfmt mismatch remains evidence-only; do not retain unrelated bulk formatting. If clippy fails only on the previously recorded untouched `chunks_exact_to_as_chunks` lint, prove M139-changed Rust test files clean and record the exact pre-existing failure rather than weakening lint policy.
...
- post-M114 roadmap is labeled historical for current-head authority after M139;
...
- historical closure files are not rewritten solely to remove historical counts;
- M061/M062 exact-path/dependency guards remain at least as strict as before;
- no broad `emissary-core/**`/`emissary-cli/src/**` allowlist is introduced;
- no dependency/feature activation appears.

## 13. Security review required at closure

Explicitly review authentication lifetime/capacity/redaction; JSON-RPC batch cardinality/authentication/notification/fanout; TLS identity/fail-closed behavior; request/task/body/deadline bounds; runtime enable/disable composition; core policy-vocabulary containment; pool target isolation/LeaseSet truthfulness; idle reason spoof-resistance; NewDest replay/race resistance; staged-secret rollback/redaction; shared-session identity/policy equality; restart/manual/failure semantics; dependency isolation and exact Yosemite pin.

No “inherited from M130/M134” row may be marked pass without current-head executable or static evidence.

## 14. Acceptance criteria

M139 closes as complete only when:

1. current implementation head is explicitly recorded;
2. M095 mechanically recomputes to `325/47/468` with 840 cells;
3. no Proposal cell is promoted by M139;
...
7. live/adversarial/production/persistence/client-services/router-info suites are green, or an environment-only skip has explicit equivalent evidence;
8. M061/M062 exact-path/dependency containment is green;
9. feature-disabled build is green;
10. active docs/registry/roadmaps agree on `325/47/468`, partial support, and M139 current authority;
11. post-M114 roadmap treats M130 as historical current-head authority superseded by M139;
12. no forbidden production source/dependency path changed under M139;
13. no unresolved high/medium correctness/security finding remains;
...
Stop and close as blocked/corrective-required if:
...
- M061/M062 can only be made green by unjustified broad allowlisting;
- dependency isolation/exact Yosemite pin drifted;
...
- full suite reveals a regression requiring runtime code;
- external/upstream mutation would be required.

When stopped for a production defect, write a narrowly scoped corrective plan naming the defect and exact owner. Do not fix it inside M139.

## 16. Closure evidence required

`plans/closure/i2pcontrol-proposal-170/139-closure.md` must include baseline/current commits, exact M139 changed files, before-edit failure/contradiction ledger, classification of each stale guard changed, requirement-to-evidence matrix, M095 recomputation/hash, exact 47-cell inventory, M061 upstream diff, M062 dependency/pin evidence, M127–M129 current-head security results, M135/M136/M137/M134 focused + integrated lifecycle results, live/adversarial/persistence/client-services/router-info results, feature-disabled/workspace checks, clippy/rustfmt/diff-check outcomes, failure/restart/contention review, compatibility/migration review, security review, documentation reconciliation, unresolved findings, explicit M130 authority disposition, and internal-only/read-only-upstream attestation.

## 17. Successor policy

A successful M139 closure establishes a clean `325/47/468` current-head baseline and leaves Proposal 170 partial.

M139 must **not** choose or register the next residual capability cluster during implementation or closure. Selection of the next cluster is a separate planning decision after requalification evidence is available.

Historical M132/M133 blocked attempts, M130 qualification, and the unused contemplated NewDest-corrective label “M138” remain preserved as historical references.
